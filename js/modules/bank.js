...
export async function depositItem(db, uid, itemName) {
    ...
    try {
        await runTransaction(db, async (ts) => {
            ...
            // Proteksi kapasitas Grid Perfect World (4x8 = 32 Slot Unik)
            const currentBankSlots = Object.keys(bankInv).length;
            if (!bankInv[itemName] && currentBankSlots >= 32) {
                throw "💥 Gagal! Brankas penuh. Slot Bank Anda telah mencapai batas maksimal grid 4x8 (32 Slot).";
            }
