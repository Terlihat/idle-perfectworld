...
window.renderQuestUI = function (questData) {
    ...
    if (!btnTake) return; // Mencegah error jika HTML belum dimuat

    const today = new Date().toLocaleDateString('id-ID');

    // 1. KONDISI: Pemain Baru atau Hari Berganti (Waktunya Ambil Misi Baru)
    if (!questData || questData.lastReset !== today) {
        btnTake.style.display = 'block'; // NYALAKAN TOMBOL AMBIL MISI
    ...
    if (questData.daily) {
        ...
        if (dq.isClaimed) {
            if (btnClaimD) {
                btnClaimD.style.display = 'inline-block';
                btnClaimD.innerText = "Selesai";
                btnClaimD.disabled = true;
                btnClaimD.style.background = "#555";
                btnClaimD.style.color = "#888";
            }
        } else if (dq.progress >= dq.target) {
            if (btnClaimD) {
                btnClaimD.style.display = 'inline-block';
                btnClaimD.innerText = "Klaim Hadiah";
                btnClaimD.disabled = false;
                btnClaimD.style.background = "#ffca28";
                btnClaimD.style.color = "#000";
            }
        } else {
            if (btnClaimD) btnClaimD.style.display = 'none';
        }
    ...
    if (questData.bounty) {
        ...
        if (bq.isClaimed) {
            if (btnClaimB) {
                btnClaimB.style.display = 'inline-block';
                btnClaimB.innerText = "Selesai";
                btnClaimB.disabled = true;
                btnClaimB.style.background = "#555";
                btnClaimB.style.color = "#888";
            }
        } else if (bq.progress >= bq.target) {
            if (btnClaimB) {
                btnClaimB.style.display = 'inline-block';
                btnClaimB.innerText = "Klaim Hadiah";
                btnClaimB.disabled = false;
                btnClaimB.style.background = "#ffca28";
                btnClaimB.style.color = "#000";
            }
        } else {
            if (btnClaimB) btnClaimB.style.display = 'none';
        }
